...
avg_salary_per_employee AS 
(SELECT 
	salary
FROM employees
WHERE department = 'Legal'
),
avg_fte_per_employee AS
(SELECT 
	id,
	fte_hours 
FROM employees
...
SELECT 
	...
	salary,
	fte_hours,
	avg_fte_per_employee / (CAST(avg_legal_fte AS REAL)),
	avg_salary_per_employee / CAST(avg_legal_salary AS REAL)
FROM employees

SELECT e.id, e.first_name, e.last_name, e.department, e.salary, e.fte_hours,
       e.salary / d.avg_salary AS salary_ratio,
       e.fte_hours / d.avg_fte_hours AS fte_hours_ratio
FROM employees e
INNER JOIN (
